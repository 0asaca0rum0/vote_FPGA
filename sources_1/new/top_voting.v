...
module top_voting(
    ...
    // Hz Generators
    onehz uno(.clk_100mhz(clk_100MHz), .clk_1hz(w_1Hz));
    twohz dos(.clk_100mhz(clk_100MHz), .clk2hz(w_2Hz));
    // Button Debouncers
    debounce_btn b1(.clk(clk_100MHz), .btn_in(btn_1), .btn_out(w_btn1));
    debounce_btn b2(.clk(clk_100MHz), .btn_in(btn_2), .btn_out(w_btn2));
    debounce_btn b3(.clk(clk_100MHz), .btn_in(btn_3), .btn_out(w_btn3));
    debounce_btn bovcv(.clk(clk_100MHz), .btn_in(btn_ov_cv), .btn_out(w_btn_ov_cv));
    debounce_btn rst(.clk(clk_100MHz), .btn_in(reset), .btn_out(w_reset));
